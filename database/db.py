...
def generate_pickup(slot, order, total_weight):
    ...
    current_order = {"vechicle_type": get_vehicle_type(current_carrier),"delivery_partner_id": 1,"list_of_order_ids_assigned": []}
        ...
        if (total_weight <= 0):
            ...
            if (i > current_carrier_capacity):
                ...
                if (current_carrier_capacity <= 0 and current_carrier_trip > 0):
                    current_order = {"vechicle_type": get_vehicle_type(current_carrier),"delivery_partner_id": orderId,"list_of_order_ids_assigned": []}
                    orderId += 1
                    current_carrier_capacity = capacity_per_trip[current_carrier - 1]
...
def get_order_delivery(slot, order):
    total_weight = 0
    for i in order:
        total_weight += i
        
    res = generate_pickup(slot, order, total_weight)
    return res
